...
export default function AIAssistantUI() {
  ...
  const API_BASE = process.env.NEXT_PUBLIC_API_URL || " https://autoplastic-mario-peakless.ngrok-free.dev";
  ...
  async function sendMessage(convId, content) {
    ...
    const aiResponse =
      data?.final_summary || "No response generated.";
      ...
      setConversations((prev) =>
        prev.map((c) => {
          ...
          const errorMsg = {
            id: Math.random().toString(36).slice(2),
            role: "assistant",
            content: "⚠️ Error fetching response from API",
            createdAt: new Date().toISOString(),
          }
